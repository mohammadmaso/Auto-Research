...
import os
import string
from actions import article_scrape
...
class ResearchAgent:
    def __init__(self, question, agent, agent_role_prompt, websocket):
        ...
        self.agent = agent
        self.agent_role_prompt = agent_role_prompt if agent_role_prompt else prompts.generate_agent_role_prompt(
            agent)
    ...
    async def async_search(self, query):
        ...
        new_search_urls = find_pdf_files_in_folder(self.resource_path)
    ...
    async def run_search_summary(self, query):
        ...
        os.makedirs(os.path.dirname(
            f"./outputs/{self.directory_name}/research-{query}.txt"), exist_ok=True)
        write_to_file(
            f"./outputs/{self.directory_name}/research-{query}.txt", result)
        return result
    ...
    async def conduct_research(self):
        ...
        self.research_summary = read_txt_files(
            self.dir_path) if os.path.isdir(self.dir_path) else ""
    ...
    async def create_concepts(self):
        ...
        """
        result = self.call_agent(prompts.generate_concepts_prompt(
